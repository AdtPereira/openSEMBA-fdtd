...
Overview
--------
This module provides Python classes to interface with the OpenSEMBA-FDTD solver. It automates common tasks such as:
- Loading and parsing `.fdtd.json` case files
- Running FDTD simulations
- Reading and managing simulation probes
- Creating new cases programmatically
- Handling mesh and material data from VTK files

Classes
-------
- Probe: Parses output files generated by FDTD simulations.
- FDTD: Interface to manage .fdtd.json input files, execute simulations, and clean up results.
- CaseMaker: Helper class to programmatically create new .fdtd.json simulation case files.
...
Probe
-----
Represents a measurement probe output from a simulation.

Attributes:
- filename: Path to the probe file.
- case_name: Simulation case name inferred from filename.
- name: Probe name.
- domainType: 'time' or 'frequency' based on file content.
- data: pandas DataFrame with loaded numerical data.
- type: Probe type (e.g., 'wire', 'point', 'mtln', 'bulkCurrent', 'farField', or 'movie').

Methods:
- __getitem__(key): Access columns from data directly.
- Various static methods to parse filename information and determine probe properties.

FDTD
----
Manages a full .fdtd.json simulation case.

Attributes:
- _filename: Path to the .fdtd.json input file.
- _input: Parsed JSON content from the input file.
- run_command: Command list to launch the simulation.
- _hasRun: Indicates if the solver has been executed.

Methods:
- getFolder(): Returns the folder containing the input file.
- getCaseName(): Returns the base name of the simulation case.
- __getitem__(key): Access items from the input JSON.
- run(): Executes the solver.
- hasFinishedSuccessfully(): Checks if the solver run completed successfully.
- cleanUp(): Deletes solver output files and folders.
- getSolvedProbeFilenames(probe_name): Returns filenames matching a probe name.
- getVTKMap(), getCurrentVTKMap(): Locate VTK output files.
- getMaterialProperties(materialName): Retrieves material properties by name.

CaseMaker
---------
Facilitates programmatic generation of new FDTD simulation cases.

Attributes:
- input: JSON dictionary to be exported to a .fdtd.json file.

Methods:
- setNumberOfTimeSteps(steps): Sets the number of simulation steps.
- setGridFromVTK(path_to_grid): Sets the spatial grid from a VTK mesh.
- setAllBoundaries(boundary_type): Assigns a boundary type.
- addCellElementsFromVTK(path_to_vtk): Adds elements from VTK.
- addCellElementBox(boundingBox): Adds a box-shaped region.
- addNodeElement(position): Adds a node at a location.
- addPECMaterial(): Adds a PEC material.
- addMaterialAssociation(materialId, elementIds): Associates materials to elements.
- addPlanewaveSource(), addPointProbe(), addFarFieldProbe(): Adds sources and probes.
- exportCase(case_name): Exports the case as a .fdtd.json file.
- Private helper methods for grid and coordinate transformations.

Notes
-----
- Filename parsing assumes standard output naming conventions.
- VTK files are critical for grid and mesh information.
- The solver can optionally run using MPI parallelization.
...
class FDTD():
    ...
    def cleanUp(self):
        """
        Removes all output files and subfolders generated by previous FDTD simulations.

        This method ensures that the working directory is clean before starting a new simulation,
        avoiding interference from old data files or output folders.

        The cleanup process includes:
        - Deleting files with the following extensions:
        - `.dat`: Probe data files.
        - `.pl`: Log files.
        - `.txt`: Text logs or reports.
        - `.xdmf`: XDMF metadata for binary outputs.
        - `.bin`: Binary output files.
        - `.h5`: HDF5 output files.
        - Removing any subdirectories present in the folder (e.g., mapVTK folders).

        Notes
        -----
        - If a file cannot be deleted (e.g., due to permission issues), an exception will be raised.
        - Subfolders are removed recursively (`ignore_errors=True`).
        - This method is safe to call multiple times, even if the folder is already empty.

        Examples
        --------
        >>> solver = FDTD('examplesData/cases/pw-in-box.fdtd.json')
        >>> solver.cleanUp()
        # Now the case folder is clean and ready for a fresh simulation run.

        See Also
        --------
        FDTD.run : Method that runs the solver after clean-up.
        """
    ...
    def getSolvedProbeFilenames(self, probe_name):
        """
        Retrieves the list of probe output filenames generated by the solver for a specific probe name.

        This method searches the current working directory for simulation output files
        that match the given `probe_name` and case name prefix. 
        ...
        ----------
        probe_name : str
            Name of the probe as defined in the simulation input file (`.fdtd.json`).
            Example: "inbox", "wire_start", "mid_point", etc.

        Returns
        -------
        List[str]
            A sorted list of filenames (relative paths) corresponding to the requested probe.
            The list may contain multiple files if the probe generates multiple output types.

        Raises
        ------
        ValueError
            If the solver input (`self._input`) does not contain any probes section.

        Notes
        -----
        - The search is performed in the current working directory (usually set to the case folder during execution).
        - Filenames are expected to follow the naming convention: 
        `<caseName>_<probeName>_<additional_info>.<extension>`.
        - Files are sorted alphabetically before returning.

        Examples
        --------
        >>> solver = FDTD('examplesData/cases/pw-in-box.fdtd.json')
        >>> solver.run()
        >>> filenames = solver.getSolvedProbeFilenames('inbox')
        >>> print(filenames)
        ['pw-in-box.fdtd_inbox_Wz_10_10_10_s1.dat']

        See Also
        --------
        FDTD.run : Runs the simulation, producing these output files.
        FDTD.cleanUp : Removes previous output files to avoid mixing old results.
        """
